...
export function InitialRequest() {
    ...
    function getRequests() {
        ...
        for (var i = 0; i < ContractorLocationInfo.length; i++) {
            ...
            var contractorLongitude = ContractorLocationInfo[i].longitude * Math.PI / 180;
            var contractorLatitude = ContractorLocationInfo[i].latitude * Math.PI / 180;
            ...
            var a = Math.pow(Math.sin(latitudeDistance / 2), 2)
                + Math.cos(contractorLatitude) * Math.cos(customerLatitude)
    ...
    useEffect(() => { //this is being called twice when you refresh the page
        if (!isloaded) {
            getRequests();
